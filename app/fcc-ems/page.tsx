...
export default function FccEmsPage() {
  ...
  const handleVerify = useCallback(() => {
    ...
    setTimeout(() => {
      ...
      if (valid) {
        ...
        setScreen('viewing');
        logEvent('fcc_ems_verified', { method: accessMethod });
      } else {
        setError(
  ...
  }, [verifying, accessMethod, codeInput]);
    ...
    return (
      <main className="min-h-screen bg-slate-900 text-white">
        ...
        <div className="text-center px-4 pt-8 pb-4">
          ...
          <p className="text-xs font-bold tracking-widest text-slate-400 font-mono">CEG.SFG.AC/FCC-EMS</p>
          ...
          <p className="text-xs text-slate-400 mt-0.5">28100 Pacific Coast Hwy, Malibu</p>
          <p className="text-xs text-slate-400">2 registered members</p>
        </div>
        ...
        {MOCK_HOUSEHOLD.access.hazards && (
          <div className="mx-4 mb-4 bg-gradient-to-r from-red-950 to-red-900 rounded-lg px-3 py-2 flex items-center gap-2 justify-center">
            ...
            <span className="text-xs font-bold text-red-300">{MOCK_HOUSEHOLD.access.hazards}</span>
          </div>
        )}
  ...
  const member = household.members[activeMember];
